...
test('Klik op alle links en buttons – rapporteer 404s', async ({ browser }) => {
  ...
  for (const target of targets as TargetElement[]) {
    if (target.type === 'link' && target.href && !target.href.startsWith('javascript:') && target.href !== '#') {
      const href = new URL(target.href, BASE_URL).toString();
    ...
    try {
      // Verhoog lading met retry-logica
      let retries = 3;
